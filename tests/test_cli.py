...
class TestEmit:
    ...
    def test_emit_success(self, mock_post: MagicMock, mock_ensure: MagicMock, tmp_path: Path, monkeypatch: object) -> None:
        ...
        result = runner.invoke(app, ["emit", "test_event", "--data", '{"msg": "hi"}'])
        assert result.exit_code == 0
    ...
    def test_emit_with_delay(self, mock_post: MagicMock, mock_ensure: MagicMock, tmp_path: Path, monkeypatch: object) -> None:
        monkeypatch.setattr("trigr.cli.Path.cwd", lambda: tmp_path)  # type: ignore[attr-defined]
        mock_post.return_value = MagicMock(status_code=200)
        result = runner.invoke(app, ["emit", "delayed", "--delay", "10s"])
        assert result.exit_code == 0
...
class TestWatch:
    ...
    def test_watch_returns_event(self, mock_get: MagicMock, mock_ensure: MagicMock, tmp_path: Path, monkeypatch: object) -> None:
        monkeypatch.setattr("trigr.cli.Path.cwd", lambda: tmp_path)  # type: ignore[attr-defined]
        mock_get.return_value = MagicMock(
            status_code=200,
            json=lambda: {"type": "test", "data": {}, "source": "", "timestamp": "2025-01-01T00:00:00"},
        )
